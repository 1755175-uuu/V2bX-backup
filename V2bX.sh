...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/1755175-uuu/V2bX-backup/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/1755175-uuu/V2bX-backup/main/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/V2bX -N --no-check-certificate https://raw.githubusercontent.com/1755175-uuu/V2bX-backup/main/V2bX.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/1755175-uuu/V2bX-backup ---
  ${green}0.${plain} 修改配置
————————————————
